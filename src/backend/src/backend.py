...
from utils.sign_up import sign_up_op, check_sign_up, insert_missions
...
from utils.get_info import get_missions, get_user_stats, get_last_user_activities, get_week_themes  
...
async def sign_up(sign_up_json: SignUpInput, request: Request) -> BooleanResponse:
    ...
    async with Connection.get_connection() as connection:
        ...
        await insert_missions(sign_up_json.email, connection)
...
async def get_profile_info(request: Request, user_id: Optional[int] = Query(None)) -> ProfileInfoResponse:
    ...
    async with Connection.get_connection() as connection:
        ...
        return ProfileInfoResponse(user_data = user_data, user_question = user_question, user_answer = user_answer, user_activities = user_last)
...
async def edit_profile(request: Request, profile_json: ProfileUpdateInput) -> BooleanResponse:
    ...
    async with Connection.get_connection() as connection:
        ...
        if await check_edit_profile(profile_json.username, user_id, connection):
            return BooleanResponse(status = False, warning = "Username already in use!")
        user_id = request.session.get("user_id")
        values_text  = "username = %s, name = %s, surname = %s"
        values_tuple = (profile_json.username, profile_json.name, profile_json.surname,)
        if profile_json.bio != "":
            values_text +=", bio = %s"
