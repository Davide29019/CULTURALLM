...
async def get_your_profile_page(request: Request):
    ...
    if "user_id" not in request.session:
        ...
        async with httpx.AsyncClient(timeout = 60.0) as client:
            ...
            return templates.TemplateResponse("my_profile.html", {"request" : request, "user_data" : result['user_data'], "user_question" : result["user_question"], "user_answer" : result["user_answer"], "last_activities" : result["user_activities"]})
...
async def edit_profile(request: Request, first_name: str = Form(...), last_name: str = Form(...), username: str = Form(...), bio: Optional[str] = Form(""), location: Optional[str] = Form(""), website: Optional[str] = Form(""), birthdate: Optional[str] = Form(""), profile_picture: Optional[UploadFile] = File(None), reset_profile_picture: Optional[str] = Form(None)):
